...
# Scheduler for forward diffusion process described in https://huggingface.co/blog/annotated-diffusion
class Scheduler2D:
    def __init__(self, timesteps=300, schedule_type='linear', device='cpu', **kwargs):
        self.timesteps = timesteps
        self.device = device

        # Generate beta schedule based on type
        if schedule_type == 'linear':
            ...
            self.betas = self._quadratic_beta_schedule(timesteps)
        elif schedule_type == 'sigmoid':
            self.betas = self._sigmoid_beta_schedule(timesteps)
        else:
            raise ValueError(f"Unknown schedule_type: {schedule_type}")

        # Pre-compute all scheduler values
    ...
    def _quadratic_beta_schedule(self, timesteps):
        beta_start = 0.0001
        beta_end = 0.02
